...
**speedy_TheWeather** is a feature-rich weather plugin for **Enigma2** receivers. It provides detailed current weather information, multi-day forecasts, hourly weather data, moon phase calculations, and an integrated animated rain radar.
...
* **Current Weather & Forecasts:** 7-day weather forecast with detailed hourly data (1h, 2h, or 3h intervals).
* **Comprehensive Metrics:** High/Low temperature, "Feels Like" temperature, wind speed, wind direction, Beaufort scale, barometric pressure, humidity, UV index, rain probability, and precipitation amounts.
* **Sun & Moon Data:** Sunrise, sunset, and mathematically calculated **moon phases, moon illumination percentage, moonrise, and moonset**.
* **Integrated Rain Radar:** Animatable rain radar powered by **RainViewer** utilizing **OpenStreetMap** map tiles.
* **Weather Alerts:** Automated detection and display of severe weather warnings (gales, heavy rain, extreme heat, severe frost).
* **Multi-Location Support:** Save multiple favorite locations and switch easily using the location search feature (City + Country Code).
* **TV Overlay:** Optional current temperature display on top of live TV viewing.
* **Skinner Support:** Custom Converter (`conv_TheWeather`) and Renderer (`rend_TheWeatherPixmap`) for seamless integration into InfoBars, SecondInfoBars, and custom screens.
* **Performance & Stability:** 5-minute data caching, asynchronous background radar downloads, and full compatibility with both Python 2 and Python 3.
* **Auto-Update System:** Built-in repository update check via GitHub with a dedicated console installer script.

---

## Quick Installation (SSH / Telnet)

Run the following command in your terminal via SSH/Telnet on your Enigma2 receiver:

```bash
wget [https://raw.githubusercontent.com/speedy005/speedy_TheWeather/master/installer.sh](https://raw.githubusercontent.com/speedy005/speedy_TheWeather/master/installer.sh) -O installer.sh
chmod +x installer.sh
sh installer.sh
Available Converter Parameters (conv_TheWeather)Skinners can use the following parameters with day prefixes (e.g., Day1,TemperatureMax or Day2,WeatherText):CategoryConverter TypeDescriptionGeneralCityName of the selected locationDate & DayDayName, Date, Date_EU, Date_EU_Short, Date_USDay of the week or formatted dateTemperatureTemperatureMax, TemperatureMinMaximum / Minimum temperature in °CWindWindSpeed_KMH, WindSpeed_MS, WindSpeed_BFT, WindSpeed_MPH, WindDirection, WindDirectionDegreeWind speed metrics and directionPrecipitationRainChance, RainAmountRain probability (%) and amount (mm)Sun & UVSunChance, Sunrise, Sunset, UVIndexSunshine probability, times, and UV indexAir & PressureHumidity, PressureRelative humidity (%) and pressure (hPa)Moon DataMoonPhase, MoonIllumination, MoonRise, MoonSetCalculated moon phase name, illumination (%), rise, and set timesGraphics/TextIcon, WeatherTextWeather icon code (for renderer) and weather description textSkin Integration & XML Widgets1. 5-Day Forecast Screen Example (TheWeather_5Days)XML<screen name="TheWeather_5Days" position="center,center" size="1800,900" title="5-Day Weather Forecast" backgroundColor="#101010" flags="wfNoBorder">
...
2. InfoBar Integration Including Moon Data Example (infobar_theweather)XML<screen name="infobar_theweather" flags="wfNoBorder" position="0,0" backgroundColor="transparent">    
  <!-- Moon Details -->
  <widget source="session.CurrentService" render="Label" position="483,765" size="250,30" font="Regular;20" halign="left" transparent="1">
    <convert type="conv_TheWeather">Day1,MoonPhase</convert>
  </widget>
...
Configuration & File PathsThe plugin stores configuration files and saved locations at the following path:Plaintext/etc/enigma2/speedy_TheWeather/
RequirementsEnigma2-based receiver (OpenATV, OpenPLi, VTi, etc.)Active internet connectionCompatible Python 2 or Python 3 environmentCredits & LicenseOriginal Author: CaughtModifications & Enhancements: speedy005 / speedy005Copyright © Caught / speedy005. All rights reserved.
